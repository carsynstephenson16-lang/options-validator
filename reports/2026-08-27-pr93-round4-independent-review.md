...
- **P1-a: closed with a documented boundary.** Residual probes: A2 (rewrite the digest inside the HTML too, update `html_sha256` + `render_id`) and A3 (producer renders one dataset, snapshots another) are still ACCEPTED — but A2 is outside the post-hoc-tampering threat model (an attacker rewriting published HTML can rewrite the visible table; byte-level detection cannot distinguish), and the receipt disclosed the boundary accurately. Duplicate digest-marker injection fails closed.
...
4. **NEW-4:** `evaluate_cli` refuses an `as_of` earlier than the newest persisted dated artifact/observation (fail closed, distinct error, tested; no override flag this round).
5. Non-blocking: NEW-5 through NEW-9 and the N4 `reason` one-liner.
