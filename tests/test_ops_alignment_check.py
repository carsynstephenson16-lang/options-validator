...
class OpsAlignmentCheckTests(unittest.TestCase):
    ...
    def _repo(self, *, ahead: int = 0, behind: int = 0, branch: str = "main") -> Path:
        ...
        for index in range(max(ahead, behind)):
            (repo / f"c{index}.py").write_text(f"y = {index}\n")
            self._git(repo, "add", f"c{index}.py")
            self._git(repo, "commit", "-qm", f"commit {index}")
        # origin/main is placed by hand so the test never touches a network.
        self._git(repo, "update-ref", "refs/remotes/origin/main",
                  f"HEAD~{behind}" if behind else "HEAD")
        if ahead and behind:
            self._git(repo, "update-ref", "refs/remotes/origin/main", "HEAD~1")
            self._git(repo, "checkout", "-q", "-B", "main", "HEAD")
            (repo / "local-only.py").write_text("z = 1\n")
            self._git(repo, "add", "local-only.py")
            self._git(repo, "commit", "-qm", "local only")
        elif ahead:
            self._git(repo, "update-ref", "refs/remotes/origin/main", f"HEAD~{ahead}")
        elif behind:
