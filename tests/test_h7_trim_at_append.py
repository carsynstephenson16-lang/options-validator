...
class TrimActivationTests(unittest.TestCase):
    ...
    def _source_receipt(self, unhealthy: list[str]) -> dict:
        ...
        for s in self.full:
            if s in unhealthy:
                symbols[s] = {"symbol": s, "healthy": False,
                              "gate": "UNKNOWN", "flags": ["MISSING"]}
            else:
                symbols[s] = {"symbol": s, "healthy": True,
                              "gate": "CLEAR", "flags": []}
    ...
    def _data_gate_receipt(self, source: dict, no_go: list[str] | None = None) -> dict:
        no_go = no_go or []
        symbols = {s: {"symbol": s,
                       "verdict": "NO_GO" if s in no_go else "GO"}
                   for s in self.full}
        go_count = len(self.full) - len(no_go)
        return make_receipt("data_gate", {
            "evaluation_session": _COMPLETED_SESSION,
            "requested_run_date": _REQUESTED_RUN_DATE,
            "scope": scope_identity(),
    ...
    def _backup_receipt(self) -> dict:
        return make_receipt("backup_restore", {
    ...
    def _assert_real_store_untouched(self, before):
        ...
        self.assertEqual(
            (after.valid, after.empty, after.count),
            (before.valid, before.empty, before.count))
    ...
    def test_trim_activates_included_and_freezes_excluded_with_reasons(self):
        ...
        self.assertEqual(uni["trim_rule"],
...
class TrimGuardUnitTests(unittest.TestCase):
    ...
    def _gate_result(self, no_go=()):
        symbols = {s: {"symbol": s, "verdict": "NO_GO" if s in no_go else "GO"}
                   for s in self.full}
        result = {"whole_universe_verdict": "GO" if not no_go else "NO_GO",
                  "go_count": len(self.full) - len(no_go),
                  "universe": sorted(self.full), "symbols": symbols}
        return result
    ...
    def test_source_health_whole_universe_passes_for_included_only(self):
        ...
        report = ag.activation_preconditions(
            ...
            universe=tuple(self.full), data_gate_result=self._gate_result(),
...
class TrimReceiptValidatorTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.source_path = Path(self.tmp.name) / "source.json"
        self.gate_path = Path(self.tmp.name) / "gate.json"

    def _write(self, no_go):
        source = make_receipt("source_health", {
            "evaluation_session": _COMPLETED_SESSION,
            "requested_run_date": _REQUESTED_RUN_DATE,
            "scope": scope_identity(),
    ...
    def test_included_go_passes_even_when_whole_universe_no_go(self):
        ...
        receipt = watch.validate_data_gate_receipt(
            self.gate_path, evaluation_session=_COMPLETED_SESSION,
            names=self.full, included=included)
    ...
    def test_included_no_go_raises(self):
        ...
        with self.assertRaises(ValueError):
            watch.validate_data_gate_receipt(
                self.gate_path, evaluation_session=_COMPLETED_SESSION,
                names=self.full, included=included)
