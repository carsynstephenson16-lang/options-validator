...
and never fetches. GO requires all 12 names to have an adjusted close AND an
...
class TestWholeUniverseGo(GateBase):
    def test_all_valid_is_go_exit_0(self):
        ...
        self.assertEqual(res["go_count"], 12)
        self.assertEqual(res["no_go_count"], 0)
        for sym in watch_universe():
            self.assertEqual(res["symbols"][sym]["verdict"], "GO")
            self.assertEqual(res["symbols"][sym]["reason_codes"],
                             ["EXACT_SESSION_GO"])
...
class TestCloseFailures(GateBase):
    def test_missing_close_forces_universe_no_go(self):
        ...
        self.assertIn("CLOSE_FILE_MISSING",
                      res["symbols"]["NVDA"]["reason_codes"])
        self.assertEqual(res["whole_universe_verdict"], "NO_GO")
        self.assertEqual(self._run(), 1)

    def test_stale_close_forces_no_go(self):
        _write_close(self.close_dir, "AMD",
                     [("2026-07-08", 90.0), ("2026-07-09", 95.0)])
        res = self._eval()
    ...
    def test_future_close_rows_excluded_during_replay(self):
        ...
        _write_close(self.close_dir, "AMD",
                     [("2026-07-09", 95.0), (self.eval_iso, 100.0),
                      (nxt, 999.0)])
        res = self._eval()
    ...
    def test_eval_row_absent_but_later_rows_present_is_no_go_no_lookahead(self):
        ...
        self.assertEqual(res["symbols"]["AMD"]["verdict"], "NO_GO")
        self.assertIsNone(
            res["symbols"]["AMD"]["close"]["latest_date_at_or_before_session"])

    def test_nonfinite_close_is_no_go(self):
        _write_close(self.close_dir, "AMD",
                     [("2026-07-09", 95.0), (self.eval_iso, float("nan"))])
        res = self._eval()
    ...
    def test_duplicate_close_date_is_no_go(self):
        _write_close(self.close_dir, "AMD",
                     [(self.eval_iso, 100.0), (self.eval_iso, 101.0)])
        res = self._eval()
        self.assertIn("CLOSE_DUPLICATE_DATE",
...
class TestChainFailures(GateBase):
    ...
    def test_stale_chain_forces_no_go(self):
        ...
        self.assertEqual(res["symbols"]["AMD"]["chain"]
                         ["newest_date_at_or_before_session"], "2026-07-08")
    ...
    def test_missing_required_column_is_no_go(self):
        ...
        res = self._eval()
        self.assertIn("CHAIN_SCHEMA_MISSING",
                      res["symbols"]["AMD"]["reason_codes"])
        self.assertEqual(res["symbols"]["AMD"]["verdict"], "NO_GO")
    ...
    def test_duplicate_contract_is_no_go(self):
        ...
        _write_chain(self.chain_dir, "AMD", self.eval_iso, df)
        res = self._eval()
        self.assertIn("CHAIN_DUPLICATE_CONTRACT",
                      res["symbols"]["AMD"]["reason_codes"])
        self.assertEqual(res["symbols"]["AMD"]["verdict"], "NO_GO")
    ...
    def test_negative_liquidity_field_is_no_go(self):
        ...
        _write_chain(self.chain_dir, "AMD", self.eval_iso, df)
        res = self._eval()
        self.assertIn("CHAIN_NEGATIVE_LIQUIDITY_FIELD",
                      res["symbols"]["AMD"]["reason_codes"])
        self.assertEqual(res["symbols"]["AMD"]["verdict"], "NO_GO")
    ...
    def test_crossed_market_is_no_go(self):
        ...
        _write_chain(self.chain_dir, "AMD", self.eval_iso, df)
        res = self._eval()
        self.assertIn("CHAIN_CROSSED_MARKET",
                      res["symbols"]["AMD"]["reason_codes"])
        self.assertEqual(res["symbols"]["AMD"]["verdict"], "NO_GO")
    ...
    def test_audit_block_is_no_go(self):
        ...
        _write_chain(self.chain_dir, "AMD", self.eval_iso, df)
        res = self._eval()
        self.assertIn("CHAIN_AUDIT_BLOCK",
                      res["symbols"]["AMD"]["reason_codes"])
        self.assertEqual(res["symbols"]["AMD"]["chain"]["audit"]["verdict"],
                         "BLOCK")
        self.assertEqual(res["symbols"]["AMD"]["verdict"], "NO_GO")
...
class TestArtifactDeterminism(GateBase):
    def test_byte_identical_artifact_for_identical_inputs(self):
        ...
        again = gate.to_artifact(gate.evaluate(
            REQ, close_dir=self.close_dir, chain_dir=self.chain_dir))
...
class TestFailClosedBoundary(GateBase):
    def test_no_fetch_path_is_called_and_inputs_unchanged(self):
        ...
        with mock.patch.object(
                ta, "get_eod_chain",
                side_effect=AssertionError("get_eod_chain called")), \
             mock.patch.object(
                ta, "blind_cache_chain",
                side_effect=AssertionError("blind_cache_chain called")), \
             mock.patch.object(
                uc, "fetch_underlying_eod_av",
                side_effect=AssertionError("fetch_av called")), \
             mock.patch.object(
                uc, "fetch_underlying_eod_yahoo",
                side_effect=AssertionError("fetch_yahoo called")), \
             mock.patch.object(
                uc, "store_closes",
                side_effect=AssertionError("store_closes called")), \
             mock.patch.object(
                rt, "audit_day",
                side_effect=AssertionError("recent_topup.audit_day called")):
            res = self._eval()
        self.assertEqual(res["whole_universe_verdict"], "GO")
