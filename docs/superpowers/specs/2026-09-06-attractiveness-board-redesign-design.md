...
## Pending owner rulings (added 2026-09-06 after brief 39 review round 3 — NOT yet part of the approved text)

The approved text above is unchanged. Brief 39's third adversarial review built
the flag-on page from the brief's own code and found one mechanical guardrail
this spec did not know about, plus one interpretation worth the owner's eye.
The owner records a letter for D13 and either says nothing about I1 (accept)
or vetoes it; the brief is then re-reviewed and dispatched.

| # | Question | Options | Recommendation |
|---|---|---|---|
| D13 | `tests/test_experiments_baseline.py:92-93` (`test_production_dashboard_has_no_experiment_imports`, an AST guard whose module docstring is "Program-level isolation tests for display-only experiment wiring") forbids `attractiveness_dashboard.py` from importing or calling `experiments_dashboard` / `exp_*` at all. D5 requires the board to call the four lane builders. Which gives? | **A** — keep the guard's purpose, make it precise: the ONLY permitted experiment import/call site is the display-only gather helper `_default_experiment_lanes`, and a new assertion proves the rest of the file has none (the ranking path stays provably clean; the parity test proves the picks snapshot and source-row hashes are identical flag on/off). **B** — compute the lanes out of process: a new `experiments_dashboard` CLI mode writes `.tmp/dashboard/experiment_lanes.json` in the ritual before the board build; the dashboard only reads it (guard untouched; adds an experiments-module edit + a ritual step, and every non-ritual build shows the four columns UNAVAILABLE). **C** — drop the four experiment columns (reverses D5). | **A.** It preserves what the 2026-08-09 clause protects — the baseline ranking — with a stronger, more specific test than today's blanket import ban, and it keeps the board's build self-contained. |
| I1 | Under the flag, owner-pinned VST/AMZN are always table rows (that is how "always shown", 2026-07-16, is satisfied). Should their detail panels ALSO be force-open, as today? **What accepting I1 reverses (quoted so the ruling is on evidence):** the code comment at `options_researcher/attractiveness_dashboard.py:5598-5600` — "… the owner-pinned names stay open by standing directive" — and the test that enforces it, `tests/test_attractiveness_dashboard.py:874` `test_render_uses_details_and_fail_visible_open_attribute` (its last assertion at `:894` requires a clean pinned AMZN panel to render `open`). Under accept, that assertion becomes its flag-on opposite with a legacy-wrapped twin; under veto, nothing changes and two of the brief's zero-open assertions become 2 (measured). | Accept (default): panels open only for fail-visible statuses (STALE/BLOCKED/SKIPPED); pinned rows are the "always shown" guarantee. Veto: keep pinned panels force-open under the flag (adds ~18 fold-outs per pinned name to the visible page on a fresh day). | **Accept.** Details are "on demand" (§2.5); a pinned name's panel is one click away, its row never is. |

Owner's ruling — recorded 2026-09-07 14:54 ET from Carsyn's in-chat wording "D13 = A, I1 accept" (owner-directed in chat; recorded by the orchestrating session, provenance label owner-directed 2026-09-07): **D13 = A** (the isolation guard is made precise: `_default_experiment_lanes` is the only permitted experiment import/call site in the production dashboard, and a new assertion proves the rest of the file clean); **I1 = accept** (pinned names are always table rows; their detail panels open only for fail-visible statuses). Brief 39 (rev 8, after Codex's 2026-09-07 stop-and-report restored the §5 lane as-of rule) is written for exactly these two outcomes.
