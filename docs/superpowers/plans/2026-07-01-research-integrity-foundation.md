...
- **The spec is the source of truth:** `docs/superpowers/specs/2026-07-01-research-integrity-foundation-design.md` (committed as `d4ebe9e` on branch `phase-1a-research-integrity`, which is already checked out).
...
- **Commit after each task.** We are already on branch `phase-1a-research-integrity`; do not create another branch and do not push.
...
- Create: `research/hashing.py` — canonical JSON + SHA-256 helpers; `cost_model_snapshot()`/`cost_model_hash()`/`config_hash()`/`data_window_hash()`.
...
Append to `tests/test_research_integrity.py` (add `import tempfile` and `from research import ledger` at the top of the file with the other imports):
...
Expected: PASS — the anchoring logic already exists from Task 3, so both tests pass immediately. (If `verify` did not accept `git_clean_tracked`, this would error; confirm the signature added in Task 3 is present.)
...
def register(hypothesis_id, decision_threshold, is_result, *, data_window,
             risk_basis, notes="", run_id=None, code_sha=None, base_dir="ledger") -> str:
    body = {
        "entry_type": "run",
        "timestamp": _now(),
        ...
        "is_window": data_window.get("is_window"),
        "is_result": is_result,
        "oos_window": data_window.get("oos_window"),
        "oos_result": None,
...
class OOSGateTests(unittest.TestCase):
    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory()
        self.base = self._tmp.name
        self.clean = lambda paths: True  # simulate a committed, clean ledger
...
def reveal_oos(hypothesis_id, run_fn, *, scoreboard_fn=None, base_dir="ledger",
    ...
    frozen cost-model hash is unchanged, the global look budget is not spent, no
    prior reveal exists for this hypothesis, and the ledger is committed+clean.
    Only then does it run the (injected) backtest, assert the OOS partition, and
    append the oos_reveal record."""
    ...
    oos_trades = run_fn()
    windows.assert_oos_only([t["entry_date"] for t in oos_trades], config.IN_SAMPLE_END)
...
class CliTests(unittest.TestCase):
    ...
    def test_verify_returns_nonzero_on_tamper(self):
        ...
        jsonl = f"{self.base}/experiments.jsonl"
        open(jsonl, "w").write(open(jsonl).read().replace("x", "HACK"))
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--ledger", default="ledger", help="ledger base dir")
...
git commit -m "feat(research): integrity CLI seams (verify, trial-log)
...
Expected: PASS (ContractTests 3 OK; ScoreboardTests 3 OK).
...
Expected: FAIL — `scoreboard` still calls the deleted `_expectancy_ci` (NameError) and has no cohort guard.
...
```python
    ...
    base = 737791  # 2020-01-06 (a Monday) as an ordinal
