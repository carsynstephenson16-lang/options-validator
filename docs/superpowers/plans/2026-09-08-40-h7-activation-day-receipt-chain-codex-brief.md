...
**Date:** 2026-09-08 (rev 3 — round 1 FAIL (15 applied), round 2 PASS WITH FIXES (11 applied); see Status)
...
**Status:** READY FOR HAND-OFF (owner-approved 2026-09-08 in-session: "approve
and go forward"). Round 1 (`reports/2026-09-08-brief-40-adversarial-review-round1.md`)
returned FAIL — two blockers (chain-dir shape; untracked receipts vs. the door's
clean-tree check), four HIGH, seven MEDIUM, one LOW — all applied in rev 2 and
cited inline as **R1-n**. Round 2 (`reports/2026-09-08-brief-40-adversarial-review-round2.md`)
returned PASS WITH FIXES — two blockers rev 2 had introduced (a third
`EVIDENCE_ALLOW` copy with a pinning test; the index committed before it was
written), two HIGH, five MEDIUM, two LOW — all applied in this revision and
cited inline as **R2-n**. Dispatch, landing and ops sync remain the owner's.
...
- WP-B (small, named edits): all THREE byte-identical `EVIDENCE_ALLOW` copies —
  `tools/schwab_chain_capture.sh:65-70`, `tools/schwab_chain_intraday_capture.sh:73-78`
  and `tools/ops_alignment_check.sh:62-67` (R2-1) — add `reports/h7_data_gate_schwab`
  and `reports/h7_forward_schwab` to each. Two equality tests pin them and must
  stay green without edits: `tests/test_ops_alignment_check.py:310`
  (`test_evidence_allow_list_matches_the_capture_gate_it_predicts`) and
  `tests/test_schwab_chain_intraday.py:388`. Missing the third copy turns the
  suite red AND makes `ops_alignment_check` publish `AHEAD_CODE / WILL REFUSE`
  15 minutes before the activation-day capture.
...
| F5 | Trim mode: full 15-name receipts, every INCLUDED name healthy (source) and per-symbol GO (data gate); excluded names may be NO_GO; the included-subset verdict is computed at `h7_activation_guard.py:176-180`; the door mirrors it at `h7_schwab_window_registration.py:662-677`; the guard takes `included` as a caller-supplied parameter (`h7_activation_guard.py:119,154`). The nine names come from `load_registered_cohort()` called with NO argument — its default store is the legacy `ledger/h7_forward` (`options_researcher/h7_cohort.py:40`); against `ledger/h7_forward_schwab` it raises `CohortUnavailableError … VALID EMPTY`, which is the pre-activation state by construction (R2-8). Run-verified: included = AMD AMZN CEG ET MSFT NOW PLTR TEM VST; excluded = AVGO CRWV IREN NVDA SMCI USAR. Inference: that legacy cohort is the same nine the Schwab activation spec names | `options_researcher/h7_activation_guard.py:106-240`; spec `docs/superpowers/specs/2026-09-02-h7-schwab-activation-spec.md` |
...
| F15 | Capture-wrapper alignment guard: branch must be `main`; `origin/main` fetched with a bounded timeout; HEAD may be AHEAD of `origin/main` **only by commits whose changed paths all fall under `EVIDENCE_ALLOW`** (`ledger/facts.log ledger/h7_forward ledger/h7_forward_schwab reports/h7_receipts reports/h7_data_gate reports/h5 reports/h6_forward reports/h8_forward reports/h10 reports/ritual reports/intraday_capture reports/live_probe reports/cache_runs reports/schwab_chains reports/schwab_chains_intraday`); `reports/h7_forward_schwab` is NOT in it today, and the SAME list exists in three files (`schwab_chain_capture.sh:65-70`, `schwab_chain_intraday_capture.sh:73-78`, `ops_alignment_check.sh:62-67`), pinned equal by two tests (R2-1). The ritual's Step 8 stages `DATA_TIER_PATHS` (`daily_ritual.sh:562-563`) every day and `FULL_TIER_PATHS` (`:572-574`) only under full authority, commits, then `push_evidence_once()` (`:612-623`: fetch → merge → push, one retry, CRIT on failure). Consequence (R2-7): an unpushed activation-day evidence commit is pushed by the NEXT morning's Step 8 (fast-forward, no conflict); same-day protection of the 15:45 capture is `EVIDENCE_ALLOW` alone. `DATA_TIER_PATHS` also contains `reports/pick_tracker` and `reports/closes_receipts`, which are in NO allow-list (R2-3). (R1-2, R1-12) | `tools/schwab_chain_capture.sh:36-52,65-104,150`; `tools/schwab_chain_intraday_capture.sh:73-78`; `tools/ops_alignment_check.sh:62-67`; `tools/daily_ritual.sh:562-623` |
...
6. `evaluate_schwab_quote_age(data_gate_receipt=receipt, included_symbols=load_registered_cohort().included)` (F13) and PRINT its verdict per name. Mode A is display only and must not alter the receipt; in Mode B (an owner-typed absolute constant exists) an over-threshold included name is reported and the tool exits **3** (R2-4 — its own code, never 1) — it still writes the receipt unchanged, because the arming gate is a post-registration obligation, not a registration input (spec §"Quote-age obligation").
...
9. Exit code per owner decision **D-5** (default below): 0 when every INCLUDED name is GO; 1 when any included name is NO_GO (excluded NO_GO alone does not fail); 2 on invalid invocation / no package / unverifiable package / unlinked source receipt; 3 = Mode B quote-age over threshold on an included name (receipt still GO). The artifact and receipt are written on 0, 1 and 3 and never on 2 (R2-4). Print both verdicts so the divergence from the legacy CLI's whole-universe rule (`h7_data_gate.py:893`) is visible on every run.
...
(`_verified_data_gate_receipt()` begins at `:87`, R2-10; note the fixture builds a flat
...
- exit 1 when an included name is NO_GO while an excluded name's NO_GO alone still exits 0; the cohort in the test comes from a fixture `load_registered_cohort` patch (no-argument call, R2-8), not a literal;
- quote-age: a test that passes the RAW result to `evaluate_schwab_quote_age` and asserts `QUOTE_AGE_EVIDENCE_INVALID` (documents R1-3), then the built-receipt path asserts Mode A output present and non-blocking; Mode B (fixture config constant) reports over-threshold and exits 3 with the written receipt bytes unchanged (R2-4);
...
0. **Preconditions — nothing is written until all pass** (each a refusal with a one-line reason):
   ...
   - `git status --porcelain` EMPTY **including untracked** — byte-identical to
     ...
     remedy: `--stage-routine-evidence` stages and commits (data-tier message
     prefix) ONLY paths inside the INTERSECTION of the ritual's `DATA_TIER_PATHS`
     and the wrapper's `EVIDENCE_ALLOW` (R2-3 — `reports/pick_tracker` and
     `reports/closes_receipts` are data-tier but NOT allow-listed, and
     `closes_receipts` gains a directory every ritual morning; committing them
     would make the 15:45 capture refuse alignment and lose an irreplaceable
     capture). Any dirty path outside that intersection is a refusal, never a
     commit; the shell test asserts the intersection property;
   ...
   - Feasibility receipt (R1-7, R2-5): reuse the newest
     `reports/h7_forward_schwab/*-feasibility-cohort9.json` iff BOTH
     `receipt["source_hash"] == source_hash(paths=FEASIBILITY_SOURCE_PATHS, root=REPO_ROOT)`
     AND `receipt["config_hash"] == config_hash()` — the two values the door
     validates (`h7_schwab_window_registration.py:353`, `:338-340`). `code_sha`
     is git HEAD and is validated nowhere; comparing it would regenerate after
     every commit. Run-verified 2026-09-08: both match the on-disk receipt;
     ...
     and include it in the evidence commit (step 6).
1. `python -m options_researcher.h7_source_health --as-of <today>` (F9; `--as-of`
   ...
   receipt; if any name in `load_registered_cohort().included` (no-argument call, R2-8) has
...
2. WP-A with that source receipt. Stop on exit 1 or 2; on exit 3 print the Mode B quote-age warning through and continue (R2-4).
...
5. **Index + run log (R2-2: written BEFORE the commit).** Write the JSON index
   of receipt paths + hashes to
   `reports/h7_forward_schwab/<date>-activation-day-receipts.json` (not
   immutable; an index, not evidence) and the plain-text run log to
   `.tmp/h7_activation_day/<date>_<HHMM>.log` (`.tmp/` is gitignored,
   `.gitignore:57`, so it never dirties the tree).
6. **Evidence commit (R1-2).** Exactly one `git add` loop over an allow-list
   ...
   regenerated, and the step-5 index — each `git add` checked for non-zero exit
   ...
   wrapper itself prints (F15). If the owner does not push, the next morning's
   ritual Step 8 pushes it (F15, R2-7). All committed paths fall under
   `EVIDENCE_ALLOW` after this brief's three wrapper edits (Scope IN) — assert
   that in the shell test. After this step `git status --porcelain` must be
   empty (assert in the test): that is the door's precondition.
7. Print, verbatim, the activation command with `--source-health-receipt`,
   ...
   **Evidence template (R1-11):** write
   ...
   `feasibility_receipt_hash`, `code_commit` = HEAD after step 6,
   `last_historical_manifest_receipt_hash` from `reports/schwab_chains/<session>/manifest.json`),
   the rest as `"<OWNER TYPES>"`. The eight source/data fields are OVERWRITTEN
   by `_assemble_evidence` (`h7_schwab_manual_activate.py:150-162`), so
   pre-filling them is harmless but not load-bearing — say so in the template
   (R2-6). `code_commit` is a snapshot valid only while HEAD is unchanged: the
   door refuses "evidence code_commit disagrees with HEAD"
   (`h7_schwab_window_registration.py:941`), so print it beside
   `git rev-parse HEAD` with the line `if you commit ANYTHING before activating,
   recompute code_commit` (R2-6). Print, on its own line: `EVIDENCE TEMPLATE IS
...
correct ordering (index before commit, R2-2); `--stage-routine-evidence` refuses a dirty `reports/closes_receipts` path (R2-3); a WP-A exit 3 does not stop the chain (R2-4); exactly one `git add` site and one `git commit` site (the
...
no owner value; every committed path matches the (edited) `EVIDENCE_ALLOW`; the tree is clean after the commit.
...
`data.cache_runner.trading_days(session, session)` is empty — imported
FUNCTION-LOCALLY inside the check, precedent `h7_watch.py:241` (R2-9:
`data/cache_runner.py:55,67` imports `grpc` and the ThetaData adapter at module
level, 0.76 s measured, and neither is loaded by the capture today; no circular
import) — print exactly
...
slot `_write_parquet_once` (`:319`, call `:408`) is never reached (R1-10, R2-10). Keep the existing timing
tolerance untouched. Mirror the check defensively in `tools/schwab_chain_capture.sh`
before it touches auth, with the same output line; there is no python-free
calendar in the repo, so the mirror is one `uv run python -c` call (0.78 s
wall, Run-verified, R2-9) — cheap enough to keep, and named here so it is a
decision rather than a guess. Add Labor Day (2026-09-07)
...
- **D-6 (R1-4): the post-activation daily procedure has a hole.** Once the
  ...
  Schwab evidence (F8). Two more full-authority modules hard-code the legacy
  `reports/h7_data_gate` path and will not see a Schwab receipt in the new
  namespace: `options_researcher/ritual_receipt.py:200` and
  `options_researcher/h7_entry_preflight.py:26` (R2-11). Closing it means a
  Schwab-aware watcher binding plus those two path lookups — `h7_watch.py` is
  inside the feasibility closure, so that brief invalidates the feasibility
  receipt and needs its own regeneration. Decide whether that
...
> (rev 3, READY FOR HAND-OFF). Read it fully, then `reports/2026-09-08-h7-receipt-regeneration-status.md`
> and both adversarial-review receipts (`reports/2026-09-08-brief-40-adversarial-review-round{1,2}.md`);
> every `R1-n` / `R2-n` tag in the brief is a finding you must close as written,
> not reinterpret. Work on a new branch `codex/brief-40-activation-day-chain` from
...
> body). The chain cache is FLAT (F7). All THREE `EVIDENCE_ALLOW` copies
> change together (R2-1). Gates before the draft PR:
