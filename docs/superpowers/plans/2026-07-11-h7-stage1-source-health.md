...
**Architecture:** A read-only research-layer module (`options_researcher/h7_source_health.py`) computes health from the existing typed gate primitives (`assertions_view`, `earnings_gate`, `next_report`) — it adds NO new gate semantics, only observability. A separate owner-driven tool (`tools/h7_refresh_earnings.py`) is the ONLY mutator; it appends one record per invocation to v2 (raw evidence) or v3 (gating, with `promoted_from` citation), validating the whole store on a temp copy BEFORE touching the real file so a bad row can never poison an append-only store. No network anywhere; no crawler (recurring automation stays deferred by owner decision).
...
- [ ] **Step 1: Add the operational constant**
...
- [ ] **Step 2: Verify the freeze tests still pass (they pin values, not the key set)**

Run: `uv run python -m unittest tests.test_config_h7 -v` → expect all pass.

- [ ] **Step 3: Commit** — `feat(stage1): H7_SOURCE_HEALTH_WARN_SESSIONS operational threshold (LLM-proposed, owner may retype)`
...
- [ ] **Step 1: Write failing tests** — fixture helper mirrors `tests/test_h7_earnings.A()` plus `record_id`/`source_type`/`occurred_date` params; cover: schedule-coverage healthy (days_to_report exact); grace-with-runway → MISSING only, healthy; grace lapsing within N sessions → MISSING+STALE, unhealthy (on=2026-07-08, occurred=2026-05-27 ⇒ grace_end Sat 2026-07-11, 2 sessions left); grace expired → gate UNKNOWN, coverage none, unhealthy; no assertions → UNKNOWN+MISSING; same-fiscal-period conflict → gate UNKNOWN with empty flags, unhealthy; point-in-time invisibility (known_as_of after cutoff ⇒ MISSING); newest-assertion fields; `watch_universe()` = 12 names, no HYLN; `_sessions_between` weekend math; CLI smoke `main(["--as-of", "2026-07-11"])` against the real committed store (cutoff = close of 2026-07-10: MSFT healthy via G0005; VST/CEG/AMZN unhealthy — their Q2 schedule rows became known 21:39 UTC, after that close; all watchlist names unhealthy; exit 1); future `--as-of` refused with exit 2.
- [ ] **Step 2: Run tests, verify they fail** (`ModuleNotFoundError: options_researcher.h7_source_health`).
- [ ] **Step 3: Implement `options_researcher/h7_source_health.py`** — module docstring states read-only + Stage 1 provenance; `watch_universe()`; `_sessions_between(start, end)` via `data.cache_runner.trading_days` (sessions strictly after start, ≤ end); `_asserted_date(a)` (occurred_date for occurred else expected_date — mirrors `h7_earnings._report_date` semantics); `symbol_health(symbol, on, assertions, *, known_as_of, warn_sessions) -> dict` with keys symbol/gate/gate_reason/newest_record_id/newest_known_as_of/event_class/status/source_type/next_report/days_to_report/coverage/grace_end/grace_sessions_left/flags/healthy per the semantics block above; `main(argv) -> int` per the CLI contract (argparse, `--as-of`, fail-closed 2 on unreadable store or future date, one line per symbol, summary line, exit 1 when any unhealthy).
- [ ] **Step 4: Run the new tests → pass; run the FULL suite → pass (exit code, not grep).**
- [ ] **Step 5: Commit** — `feat(stage1): per-symbol source-health report over the v3 gating store`
...
- [ ] **Step 1: Write failing tests** — tempfile v2+v3 fixture stores (valid per loader: https URLs, tz-aware stamps, monotone checked_at, A0001 MSFT occurred sec_filing WITH fiscal identity, A0002 VST confirmed schedule company_pr with fiscal, G0001 promoting A0001); cores called with explicit paths + fixed `now_utc`. Cover: append_raw happy path (next A-id, file gains exactly one line, prior bytes untouched — append-only proof, store revalidates); aggregator+confirmed refused; aggregator+estimated+empty-notes refused; occurred+company_pr+empty-notes refused; occurred+sec_filing+empty-notes accepted; dry-run leaves file byte-identical; naive (non-tz) known-as-of → temp-copy validation refuses and real store byte-identical; regressive now_utc (earlier than store tail checked_at) → refused, store unchanged; notes containing commas round-trip. promote happy path (G-id appended, promoted_from cites raw id, `_PROMOTION_FIELDS` verbatim, `load_assertions(gating, raw)` passes); promote without fiscal as actual_quarterly_earnings refused with guidance; foreign raw id refused; double-promotion refused; `--supersedes` unknown G id refused; valid `--supersedes` honored (superseded row leaves `assertions_view`); promote dry-run writes nothing.
- [ ] **Step 2: Run tests, verify they fail.**
- [ ] **Step 3: Implement `tools/h7_refresh_earnings.py`** — sys.path shim like the collector; import `_GATING_COLUMNS`, `_RAW_COLUMNS`, `_PROMOTION_FIELDS`, loaders, `EVENT_CLASSES`, `GATING_EVENT_CLASS`, paths from `h7_earnings` (private-constant imports are deliberate: single source of truth for the contract); `_next_id(rows, prefix)`; `_validated_append(path, row, columns, loader)` (copy store to a system-temp file, append row, run loader on the copy, only then append to the real store, always unlink the temp); `append_raw(...)` and `promote(...)` cores enforcing exactly the refresher rules above, `raise SystemExit(msg)` on refusal, returning the appended row dict; `main(argv) -> int` with `append-raw`/`promote` subparsers, `--dry-run` on both, `checked_at = datetime.now(timezone.utc).isoformat()`, printed confirmation of the exact row appended.
- [ ] **Step 4: Run the new tests → pass; FULL suite → pass (exit code).**
- [ ] **Step 5: Commit** — `feat(stage1): owner-in-the-loop append-only earnings refresher (append-raw + promote under the citation contract)`

### Task 4: Docs, roadmap status, ledger fact

- [ ] **Step 1:** README + CLAUDE.md: add the two command lines next to the existing h7_watch entry. Roadmap doc: annotate Stage 1 heading `(BUILT 2026-07-11 — h7_source_health + h7_refresh_earnings; stages 2-8 still not implemented)` and adjust the top status line to match. `ledger/facts.log`: append one dated fact (mirror tail format) recording merge `2d04b47` + Stage 1 tooling with test counts.
- [ ] **Step 2:** `uv run ruff check .`, `uv run pyright`, full unittest — all exit 0 (true exit codes).
- [ ] **Step 3: Commit** — `docs+ledger(stage1): roadmap Stage 1 BUILT; commands documented; dated fact appended`

## Self-review

Spec coverage: newest assertion + class/status/source (symbol_health newest_* fields) ✓; days to expected report ✓; STALE/MISSING flags with N-session grace warning ✓; exit non-zero when unhealthy ✓; owner-in-the-loop append-only refresher with SEC-first hierarchy and disclosed aggregator estimates ✓; promotion cites raw evidence (contract re-proved on every append) ✓; no crawler/no recurring automation ✓; offline tests ✓. Type consistency: `symbol_health` field names used identically in module, CLI, and tests; `_PROMOTION_FIELDS` imported, never re-declared. No placeholders remain.
