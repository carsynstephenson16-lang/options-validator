...
| H7 Schwab restart candidate (`h7-forward-schwab-v1`) | Exact-session preclose package under `.cache/schwab_chains/` | Frozen history remains measurement input only | Read-only full Schwab chain | **STOPPED_FEASIBILITY / NOT REGISTERED / NOT ACTIVATED** | Replayed receipt projects 4.0 entries, below the required 20. The read-only capture/canary machinery remains prepared, but cannot activate v1. Any v2 experiment requires separate versioning and preregistration | `reports/h7_forward_schwab/2026-08-11-feasibility-primary-earnings.json`; `reports/h7_forward_schwab/2026-08-11-v2-arming-bottleneck-design.md` |
