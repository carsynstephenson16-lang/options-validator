...
REAL_FORWARD_STORES = frozenset({LEGACY_REAL_FORWARD_STORE, SCHWAB_REAL_FORWARD_STORE})
...
def resolve_owner_fields(base: Path) -> tuple[str, ...]:
    """Return the immutable owner-field contract for this store namespace.

    The terminal path segment is the only classifier. It works for the real
    Schwab store and temporary test stores alike, leaves every other path on
    the legacy contract, and cannot be widened by a data-gate payload, scope,
    module-level map, or caller-supplied field list.
    """
    return SCHWAB_OWNER_FIELDS if base.name == SCHWAB_FORWARD_STORE.name else LEGACY_OWNER_FIELDS
...
def _working_tree_clean() -> Check:
    ...
    return Check(
        "working_tree_clean", out == "", "clean" if out == "" else f"dirty: {out.splitlines()[:5]}"
    )
...
def activation_preconditions(
    *,
    forward_base,
    source_health_by_symbol: dict,
    universe: tuple,
    data_gate_result: dict,
    owner_inputs: dict,
    ...
    if not allow_real_readonly and any(_within(base, store) for store in REAL_FORWARD_STORES):
        raise ActivationBoundaryError("guard fixtures must use synthetic stores")
    owner_fields = resolve_owner_fields(base)

    report = GuardReport(
        forward_base=str(base),
    ...
    if included is None:
        gate_ok = data_gate_result.get("whole_universe_verdict") == "GO" and data_gate_result.get(
            "go_count"
        ...
        not_go = sorted(s for s in checked_names if per_symbol.get(s, {}).get("verdict") != "GO")
        gate_ok = bool(checked_names) and not not_go
        gate_reason = "GO for all included" if gate_ok else f"included not GO: {not_go}"
    report.checks.append(Check("data_gate_go", gate_ok, gate_reason))

    if strict:
        current_scope = scope_identity()
        scope_ok = data_gate_result.get("scope") == current_scope and tuple(
            data_gate_result.get("universe", ())
        ...
        report.checks.append(
            Check(
                "receipt_chain",
                receipts_ok,
                "source-health -> data-gate receipts linked"
                if receipts_ok
        ...
        if included is None:
            ...
            source_reason = (
                "all current names healthy" if source_ok else "source health is partial or stale"
            )
        else:
            ...
            receipt_syms = source_health_receipt.get("symbols", {}) if source_health_receipt else {}
            source_ok = (
                source_health_receipt is not None
                and bool(incl)
                and incl.isdisjoint(set(source_health_receipt.get("unhealthy_symbols") or []))
                and all(receipt_syms.get(s, {}).get("healthy") is True for s in incl)
            )
            source_reason = (
                "all included names healthy"
                if source_ok
                else "an included name is unhealthy/stale in the receipt"
            )
        report.checks.append(Check("source_health_all_names", source_ok, source_reason))
        identity_ok = (
            data_gate_receipt is not None
            and data_gate_receipt.get("config_hash") == config_hash()
            and data_gate_receipt.get("source_hash") == diagnostic_source_hash()
        )
        report.checks.append(
            Check(
                "source_config_identity",
                identity_ok,
                "code/config identity matches receipts"
                if identity_ok
        ...
        if backup_restore_receipt is not None:
            backup_ok = (
                ...
                and (
                    completed_session is None
                    or backup_restore_receipt.get("completed_session") == completed_session
                )
            )
        report.checks.append(
            Check(
                "fresh_backup_restore",
                backup_ok,
                "fresh verified restore evidence"
                if backup_ok
                else "missing/stale/unverified backup restore evidence",
            )
        )

    blank = [field for field in owner_fields if owner_inputs.get(field) in (None, "")]
    report.checks.append(
        Check("owner_inputs_complete", not blank, "complete" if not blank else f"blank: {blank}")
    )
